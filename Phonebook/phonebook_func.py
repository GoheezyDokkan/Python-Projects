...
def first_run(self):
    data = ('John','Doe','John Doe','111-111-1111','johndoe@email.com')
    conn = sqlite3.connect('phonebook.db')
    with conn:
        cur = conn.cursor()
...
def onSelect(self,event):
    ...
    select = varList.curselection()[0] # index of the list selection
    value = varList.get(select) # list selection's text value
